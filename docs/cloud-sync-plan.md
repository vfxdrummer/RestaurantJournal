...
- Add defaults to the Journal models. *(Done — Restaurant/Visit/PhotoAsset/VoiceNote + Person/DetectedFace.)*
- Split into the two-store container; wire the CloudKit config + entitlement. *(Done, with local-only fallback.)*
- Forward fields added early to avoid a second migration: `PhotoAsset.photoCloudIdentifier`, `Visit.coverThumbnailData`.

**✅ Phase 2 — Turn on sync + status surface (same-device restore)**
- `.private` CloudKit on the Journal store. *(Done.)*
- `CloudSyncStatus` service + "iCloud Backup" section in `AppSettingsView` (account status + live sync events).
- Test: create visits → delete app → reinstall → journal + full-res photos return. *(Scenario A — pending capability + device test.)*

**✅ Phase 3 — Cover thumbnails + new-device restore**
- `Visit.coverThumbnailData` generated by `CoverThumbnailService` (~320px JPEG, incremental backfill).
- `PhotoAsset.photoCloudIdentifier` captured by `SyncMaintenance` via `PhotoLibraryLinker`.
- One-time restore remap (cloud id → new local id) + scanner dedup guard (`visitClaimingAnyPhoto`).
- `PhotoThumbnailView` falls back to the synced thumbnail; list + map covers pass it.
- Test on a second device / erased device. *(Scenario B — pending device test.)*

**⏸️ Phase 4 — Keep faces local (relationship sever) — DEFERRED**
- Move `Person`/`DetectedFace` to the Local store; replace the `Visit ↔ DetectedFace` SwiftData
  relationship with a stored `visitID`, and rebuild `Person.uniqueVisits` via that id.
- *Deferred with the People feature. For now faces ride in the Journal store (relationships intact).*

**✅ Phase 5 — Settings toggle**
- "Back up to iCloud" switch (`SyncPreference`), read by the container builder at launch; off builds a
  local-only journal. Applies on next app open. *(Done.)*
